...
def match_labels(true_labels, pred_labels):
    ...
    for perm in permutations(unique_pred_labels):
...
cv_scores = cross_val_score(gmm, X_train, y_train, cv=5, scoring=scorer)
...
gmm.fit(X_train)
test_labels = gmm.predict(X)
mapped_test_labels = match_labels(y, test_labels)

cm = confusion_matrix(y, mapped_test_labels, labels=[1, 2, 3, 4])
